// GENERATED CODE - DO NOT MODIFY BY HAND

// Iconyfy Copyright (c) 2024, Mustafa US. All rights reserved.
// This code is licensed under AGPLv2 license (see LICENSE file for details)

// brandico  icon package is licensed under CC-BY-SA-3.0 https://creativecommons.org/licenses/by-sa/3.0/, {Fontello https://github.com/fontello/brandico.font}
// 
import 'package:flutter_svg/flutter_svg.dart';
import 'package:flutter/material.dart';
import "package:iconifyly/src/package_info.dart";

class Brandico {
static PackageInfo packageInfo() {
    return PackageInfo.fromJson({
		"prefix": "brandico",
		"info": {
			"name": "brandico",
			"total": 45,
			"author": {
				"name": "Fontello",
				"url": "https://github.com/fontello/brandico.font"
			},
			"license": {
				"title": "CC BY SA",
				"spdx": "CC-BY-SA-3.0",
				"url": "https://creativecommons.org/licenses/by-sa/3.0/"
			},
			"samples": [
				"vimeo",
				"twitter-bird",
				"yandex",
				"bandcamp",
				"facebook",
				"win8"
			],
			"category": "Logos"
		},
		"lastModified": 1722793005,
		"width": 1000,
		"height": 1000
	});
}

static Widget getIcon(String name,{double? width,double? height,Color? color}) {
    return Brandico.names[name]!(width:width,height:height,color:color);
}


static Widget amex({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("amex")) {
        body = Brandico.iconList["amex"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget bandcamp({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("bandcamp")) {
        body = Brandico.iconList["bandcamp"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget blogger({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("blogger")) {
        body = Brandico.iconList["blogger"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget bloggerRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("blogger-rect")) {
        body = Brandico.iconList["blogger-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget box({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 704 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("box")) {
        body = Brandico.iconList["box"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget boxRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("box-rect")) {
        body = Brandico.iconList["box-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget codepen({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 988 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("codepen")) {
        body = Brandico.iconList["codepen"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget deviantart({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("deviantart")) {
        body = Brandico.iconList["deviantart"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget diigo({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 500 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("diigo")) {
        body = Brandico.iconList["diigo"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget discover({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("discover")) {
        body = Brandico.iconList["discover"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget facebook({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("facebook")) {
        body = Brandico.iconList["facebook"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget facebookRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("facebook-rect")) {
        body = Brandico.iconList["facebook-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget friendfeed({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("friendfeed")) {
        body = Brandico.iconList["friendfeed"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget friendfeedRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("friendfeed-rect")) {
        body = Brandico.iconList["friendfeed-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget github({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("github")) {
        body = Brandico.iconList["github"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget githubText({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("github-text")) {
        body = Brandico.iconList["github-text"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget googleplusRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("googleplus-rect")) {
        body = Brandico.iconList["googleplus-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget houzz({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("houzz")) {
        body = Brandico.iconList["houzz"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget icq({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("icq")) {
        body = Brandico.iconList["icq"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget instagram({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("instagram")) {
        body = Brandico.iconList["instagram"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget instagramFilled({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("instagram-filled")) {
        body = Brandico.iconList["instagram-filled"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget jabber({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("jabber")) {
        body = Brandico.iconList["jabber"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget lastfm({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("lastfm")) {
        body = Brandico.iconList["lastfm"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget lastfmRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("lastfm-rect")) {
        body = Brandico.iconList["lastfm-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget linkedin({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("linkedin")) {
        body = Brandico.iconList["linkedin"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget linkedinRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("linkedin-rect")) {
        body = Brandico.iconList["linkedin-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget mastercard({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("mastercard")) {
        body = Brandico.iconList["mastercard"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget odnoklassniki({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("odnoklassniki")) {
        body = Brandico.iconList["odnoklassniki"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget odnoklassnikiRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("odnoklassniki-rect")) {
        body = Brandico.iconList["odnoklassniki-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget picasa({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("picasa")) {
        body = Brandico.iconList["picasa"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget skype({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("skype")) {
        body = Brandico.iconList["skype"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget tudou({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 998 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("tudou")) {
        body = Brandico.iconList["tudou"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget tumblr({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("tumblr")) {
        body = Brandico.iconList["tumblr"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget tumblrRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("tumblr-rect")) {
        body = Brandico.iconList["tumblr-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget twitter({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("twitter")) {
        body = Brandico.iconList["twitter"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget twitterBird({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("twitter-bird")) {
        body = Brandico.iconList["twitter-bird"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget vimeo({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("vimeo")) {
        body = Brandico.iconList["vimeo"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget vimeoRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("vimeo-rect")) {
        body = Brandico.iconList["vimeo-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget visa({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("visa")) {
        body = Brandico.iconList["visa"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget vkontakteRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("vkontakte-rect")) {
        body = Brandico.iconList["vkontakte-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget win8({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 986 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("win8")) {
        body = Brandico.iconList["win8"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget wordpress({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("wordpress")) {
        body = Brandico.iconList["wordpress"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget yandex({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("yandex")) {
        body = Brandico.iconList["yandex"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget yandexRect({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("yandex-rect")) {
        body = Brandico.iconList["yandex-rect"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 

static Widget youku({double? width,double? height,Color? color}) { 
       var svg_start = '<svg xmlns="http://www.w3.org/2000/svg" width="1em" viewBox="0 0 24 24" height="1em" version="1.1">';
    var body = "";
    if(Brandico.iconList.containsKey("youku")) {
        body = Brandico.iconList["youku"]!;
    }
    return SvgPicture.string("${svg_start}${body}</svg>",width: width,height: height,colorFilter:color != null ? ColorFilter.mode(color!, BlendMode.srcIn) : null);
    } 


static Map<String,Widget Function({double? width,double? height,Color? color})> names  =   {

    "amex":amex,
    "bandcamp":bandcamp,
    "blogger":blogger,
    "blogger-rect":bloggerRect,
    "box":box,
    "box-rect":boxRect,
    "codepen":codepen,
    "deviantart":deviantart,
    "diigo":diigo,
    "discover":discover,
    "facebook":facebook,
    "facebook-rect":facebookRect,
    "friendfeed":friendfeed,
    "friendfeed-rect":friendfeedRect,
    "github":github,
    "github-text":githubText,
    "googleplus-rect":googleplusRect,
    "houzz":houzz,
    "icq":icq,
    "instagram":instagram,
    "instagram-filled":instagramFilled,
    "jabber":jabber,
    "lastfm":lastfm,
    "lastfm-rect":lastfmRect,
    "linkedin":linkedin,
    "linkedin-rect":linkedinRect,
    "mastercard":mastercard,
    "odnoklassniki":odnoklassniki,
    "odnoklassniki-rect":odnoklassnikiRect,
    "picasa":picasa,
    "skype":skype,
    "tudou":tudou,
    "tumblr":tumblr,
    "tumblr-rect":tumblrRect,
    "twitter":twitter,
    "twitter-bird":twitterBird,
    "vimeo":vimeo,
    "vimeo-rect":vimeoRect,
    "visa":visa,
    "vkontakte-rect":vkontakteRect,
    "win8":win8,
    "wordpress":wordpress,
    "yandex":yandex,
    "yandex-rect":yandexRect,
    "youku":youku,

};
static Map<String,String> iconList  =   {

    "amex":'<path fill="currentColor" d="M46.531 186C20.841 186 0 206.814 0 232.5v535C0 793.189 20.844 814 46.531 814H953.47c25.69 0 46.531-20.814 46.531-46.5v-535c0-25.689-20.844-46.5-46.531-46.5H46.53zM37.22 317.938h72.875l9.406 19.312v-19.313h85.438l18.687 42.25l18.219-42.25h271.312c12.332 0 23.424 2.282 31.563 9.532v-9.531h74.25v9.53c12.79-7.033 28.611-9.53 46.625-9.53H773.22l9.968 19.312v-19.313h79.688l10.875 19.313v-19.313h77.625v163.688h-78.438l-14.843-24.781v24.781H760.22l-10.657-26.094H725.47l-10.875 26.094H663.78c-19.935 0-35.072-4.73-44.812-9.969v9.969h-120.75v-37.156c0-5.243-.93-5.547-4.094-5.656h-4.531l.125 42.812h-233.5v-20.25l-8.375 20.313h-48.781l-8.375-20v19.937H96.75L86 455.531H61.906l-10.781 26.094H3.375V395.75zM3.375 459.03h30.344l10.843-26.25h58.126l10.718 26.25h56.813v-92.187l40.594 92.187h24.53l40.47-92.094l.125 92.094h28.625V341.437h-46.5L224.5 421.22l-36.563-79.781h-45.562V452.75l-47.5-111.313h-41.5l-50 117.594zm322.156-117.594V459.03h93.125v-24.344h-65.25V411.22h63.688v-24.094h-63.688v-21.188h65.25v-24.5h-93.125zm113.25 0V459.03h28v-42.78h30.031c10.076 0 16.317.896 20.407 5.031c5.092 5.657 4.406 15.831 4.406 22.906l.125 14.844h28.219v-23.125c0-10.5-.674-15.714-4.625-21.562c-2.49-3.426-7.72-7.566-13.844-9.906c7.26-2.909 19.719-12.597 19.719-31.344c0-13.115-5.334-20.78-14.188-26.031c-9.047-5.253-19.572-5.625-33.875-5.625h-64.375zm128.375 0V459.03h28.5V341.437h-28.5zm99.438 0c-19.258 0-33.403 4.335-42.438 13.72c-12.01 12.596-15.062 28.53-15.062 45.968c0 21.382 5.116 34.892 14.937 44.875c9.74 9.983 26.916 13.031 40.5 13.031h32.719l10.563-26.25h58.03l10.876 26.25h56.906v-88.25l52.969 88.25h39.625V341.437h-28.625v81.907l-49.125-81.906h-42.563v111.218l-47.312-111.219h-41.875l-39.375 92.47H664.78c-7.356 0-15.166-1.41-19.468-6.063c-5.195-5.934-7.688-15.028-7.688-27.625c0-12.324 3.267-21.674 8.031-26.625c5.547-5.44 11.302-6.969 21.5-6.969h26.5v-25.188h-27.062zM73.375 360.75l18.938 47.031h-37.75zm664.031 0l19.344 47.031h-38.594zm-270.937 5.031H501c4.885 0 9.97.23 13.281 2.125c3.636 1.708 5.875 5.355 5.875 10.375c0 5.123-2.145 9.227-5.781 11.125c-3.852 2.337-8.386 2.438-13.844 2.438H466.47V365.78zM200.53 519.5h150.563l21.594 25.031L395.78 519.5h100.781c11.653 0 30.779 1.207 39.375 9.563V519.5H626c8.463 0 26.79 1.695 37.656 9.563V519.5h136.532v9.563c6.788-6.538 21.147-9.563 33.375-9.563h76.468v9.563c8.026-5.83 19.337-9.563 34.938-9.563h51.718v23.344h-55.656c-29.311 0-40.062 17.89-40.062 34.906c0 37.124 32.71 35.417 58.937 36.313c4.864 0 7.817.803 9.844 2.593c2.055 1.628 3.406 4.364 3.406 7.782c0 3.201-1.335 5.841-3.281 7.656c-2.245 2.2-5.983 2.906-11.313 2.906h-53.718v25.156h53.937c17.899 0 31.121-5.073 37.907-15.031v30.5c-11.191 5.427-25.908 7.563-40.97 7.563h-72.374v-10.5c-8.386 6.753-23.514 10.5-38 10.5H617.156v-37.844c0-4.638-.47-4.844-5.093-4.844h-3.594v42.688h-75.125V639c-12.573 5.455-26.824 5.937-38.907 5.75h-8.968v38.5l-91.063-.063l-22.375-25.468l-23.781 25.468H200.531V519.5zm25.156 23.5v117.594h110.72l35.78-38.938l34.407 38.938h54.344v-39.438h34.875c24.433 0 48.687-6.744 48.687-39.406c0-32.563-24.942-38.75-47.125-38.75h-89.313L372.5 581.563L338.125 543H225.687zm331.407 0v117.594h28.031v-42.938h29.844c10.186 0 16.42 1.007 20.5 5.219c5.19 5.467 4.5 15.926 4.5 23v14.719H668V637.28c-.113-10.364-.682-15.728-4.625-21.468c-2.382-3.426-7.44-7.567-13.656-9.875c7.35-2.936 19.777-12.515 19.781-31.282c0-13.413-5.531-20.736-14.344-26.093c-9.164-4.922-19.488-5.563-33.625-5.563zm128.343 0v117.594h93.125v-24.407l-65.343-.093v-23.407h63.75v-24h-63.75v-21.375h65.343V543zm146.438 0c-29.264 0-39.906 17.945-39.906 35c0 37.21 32.708 35.51 58.812 36.406c4.861 0 7.803.831 9.938 2.625c1.916 1.631 3.406 4.325 3.406 7.75c0 3.21-1.393 5.869-3.313 7.688c-2.352 2.205-6.08 2.906-11.375 2.906H795.28v25.219h54.375c28.151 0 43.844-11.83 43.844-37.125c0-12.102-2.909-19.244-8.344-24.907c-6.322-6.446-16.721-9.065-31.875-9.437l-15.031-.406c-4.755 0-7.93-.312-10.875-1.344c-3.512-1.305-6.094-4.314-6.094-8.75c0-3.73 1.13-6.565 4.188-8.469c2.807-1.932 6.112-2.125 11.187-2.125h51.188V543zm-398.969 12.594v94.062l-42.781-47.781zm-179.531 11.781h70.094l30.594 34.313l-31.938 34.53h-68.75v-23.405h61.375V588.78h-61.375zm207.219 0h36.562c10.12 0 17.125 4.149 17.125 14.531c0 10.268-6.698 15.688-17.468 15.688h-36.22v-30.219zm125 0h34.531c4.99 0 9.983.105 13.375 2.094c3.632 1.877 5.781 5.446 5.781 10.406s-2.149 8.954-5.781 11.125c-3.749 2.199-8.39 2.406-13.844 2.406h-34.062zm360.187.813h50.907v33.593c-.78-1.01-1.463-2.005-2.594-2.906c-6.216-6.469-16.416-9.12-31.688-9.5l-15.125-.406c-4.651 0-7.835-.308-10.781-1.344c-3.619-1.31-6.094-4.327-6.094-8.781c0-3.746 1.148-6.588 4.094-8.5c2.844-1.94 6.2-2.157 11.281-2.157z"/>',
    "bandcamp":'<path fill="currentColor" d="M0 791.667h694.444L1000 208.333H333.334z"/>',
    "blogger":'<path fill="currentColor" d="M550.422 0L249.556 1.285C122.233 18.801 13.145 97.717 3.51 232.349c-3.046 12.359-1.544 24.907-1.975 36.389C.147 327.571.48 390.867 1.379 451.439c.24 95.584-2.583 172.602-.783 243.664c1.153 208.816 124.429 306.403 348.288 302.339c55.46-1.007 118.876-1.368 191.602.533c72.331.488 128.091 5.253 177.78-1.818c220.679-31.402 263.696-179.41 272.657-299.236V559.699c-6.44-139.393-107.508-177.77-193.922-180.601c9.288-34.845 11.521-82.219 3.385-146.029c-10.471-123.99-98.471-220.938-249.964-233.07zM298.828 187.246h244.761c135.326 18.024 128.953 163.934 0 184.864H298.828c-135.326-18.024-128.953-163.934 0-184.864m-55.572 403.798l449.653 1.536c135.976 17.536 136.359 183.07 4.733 203.325l-449.653-1.568c-139.444-17.144-137.487-182.865-4.733-203.294z"/>',
    "blogger-rect":'<path fill="currentColor" d="M196.099.156C88.379.156.219 88.317.219 196.036v607.851c0 107.72 88.16 195.849 195.88 195.849H803.95c107.72 0 195.849-88.129 195.849-195.849V196.036C999.799 88.316 911.67.156 803.95.156zm338.012 186.777c94.026 7.596 148.647 68.316 155.146 145.948c5.05 39.953 3.637 69.6-2.127 91.418c53.633 1.773 116.39 25.822 120.388 113.098v85.911c-5.561 75.026-32.259 167.679-169.225 187.339c-30.84 4.427-65.452 1.464-110.345 1.157c-45.139-1.19-84.496-.975-118.917-.344c-138.941 2.545-215.468-58.535-216.185-189.28c-1.116-44.494.65-92.734.501-152.581c-.558-37.925-.768-77.544.094-114.381c.267-7.189-.671-15.069 1.22-22.807c5.98-84.295 73.682-133.698 152.706-144.665l186.745-.814zM377.963 304.192c-80.036 13.104-83.991 104.44 0 115.726h151.924c80.036-13.104 83.991-104.44 0-115.726zm-34.508 252.82c-82.395 12.791-83.607 116.536 2.941 127.271l279.1 1.001c81.695-12.682 81.454-116.322-2.941-127.302z"/>',
    "box":'<path fill="currentColor" d="M351.455 294.781c-75.701 0-145.649 24.077-203.103 64.682V74.048C148.351 33.156 115.206 0 74.304 0S.256 33.156.256 74.048v545.421c-.727 9.233.088 62.72 2.534 77.904C27.12 868.388 173.74 1000 351.455 1000c194.75 0 352.609-157.858 352.609-352.609c-.001-194.751-157.859-352.61-352.609-352.61m0 555.713c-112.174 0-203.103-90.93-203.103-203.104s90.929-203.103 203.103-203.103c112.173 0 203.103 90.929 203.103 203.103s-90.93 203.104-203.103 203.104"/>',
    "box-rect":'<path fill="currentColor" d="M192.912 0h614.175C913.961 0 1000 86.039 1000 192.912v614.176C1000 913.962 913.961 1000 807.087 1000H192.912C86.039 1000 0 913.962 0 807.088V192.912C0 86.039 86.039 0 192.912 0m306.684 356.346c-52.991 0-101.955 16.854-142.172 45.277v-199.79c0-28.624-23.202-51.833-51.833-51.833c-28.632 0-51.833 23.209-51.833 51.833v381.794c-.509 6.463.062 43.904 1.774 54.532C272.562 757.872 375.196 850 499.596 850c136.326 0 246.826-110.501 246.826-246.826c0-136.326-110.5-246.828-246.826-246.828m0 388.999c-78.521 0-142.172-63.651-142.172-142.172c0-78.522 63.65-142.172 142.172-142.172s142.172 63.65 142.172 142.172s-63.651 142.172-142.172 142.172"/>',
    "codepen":'<path fill="currentColor" d="M987.672 343.936c-.09-.624-.246-1.221-.35-1.845c-.21-1.196-.43-2.392-.742-3.561c-.183-.702-.442-1.377-.662-2.052c-.339-1.04-.675-2.079-1.091-3.092c-.289-.702-.625-1.404-.948-2.079a33 33 0 0 0-1.443-2.806c-.39-.677-.805-1.327-1.208-1.976a38 38 0 0 0-1.796-2.575a49 49 0 0 0-1.454-1.845a39 39 0 0 0-2.119-2.287c-.556-.57-1.104-1.143-1.689-1.663a51 51 0 0 0-2.402-2.026c-.635-.494-1.261-.988-1.923-1.456c-.249-.156-.465-.364-.701-.521L517.575 13.108a42.44 42.44 0 0 0-47.102 0L18.906 314.152c-.233.156-.454.364-.7.521c-.664.467-1.288.961-1.911 1.456a39 39 0 0 0-2.403 2.026c-.598.521-1.143 1.093-1.704 1.663a37 37 0 0 0-2.104 2.287c-.506.599-1 1.223-1.469 1.845a38 38 0 0 0-1.78 2.575a37 37 0 0 0-1.221 1.976a41 41 0 0 0-1.444 2.806c-.324.675-.662 1.377-.948 2.079c-.416 1.013-.753 2.052-1.091 3.092c-.221.675-.468 1.351-.664 2.052c-.311 1.169-.519 2.365-.74 3.561c-.103.624-.26 1.221-.338 1.845A41 41 0 0 0 0 349.472v301.045c0 1.87.142 3.716.389 5.562c.078.599.235 1.221.338 1.816c.221 1.197.429 2.392.74 3.562c.196.702.443 1.377.664 2.053c.337 1.038.675 2.079 1.091 3.117c.286.705.624 1.38.948 2.056c.456.958.923 1.893 1.444 2.831c.389.651.792 1.297 1.221 1.949a38 38 0 0 0 1.78 2.575c.469.648.963 1.247 1.469 1.843c.664.778 1.366 1.56 2.104 2.288c.561.569 1.106 1.142 1.704 1.66a38 38 0 0 0 2.403 2.029a41 41 0 0 0 1.911 1.454c.246.156.467.366.7.519l451.567 301.047a42.45 42.45 0 0 0 23.558 7.146a42.45 42.45 0 0 0 23.544-7.146l451.568-301.047c.236-.152.452-.362.701-.519c.662-.466 1.288-.958 1.923-1.454a48 48 0 0 0 2.402-2.029c.585-.519 1.134-1.091 1.689-1.66a39 39 0 0 0 2.119-2.288a33 33 0 0 0 1.454-1.843a38 38 0 0 0 1.796-2.575c.403-.652.818-1.298 1.208-1.949c.532-.938.998-1.873 1.443-2.831c.323-.676.659-1.351.948-2.056c.416-1.038.752-2.079 1.091-3.117c.22-.676.479-1.351.662-2.053c.312-1.171.532-2.365.742-3.562c.104-.596.26-1.218.35-1.816c.232-1.847.379-3.692.379-5.562V349.471a43 43 0 0 0-.378-5.535M536.484 127.768l332.657 221.755l-148.588 99.403l-184.069-123.13zm-84.917 0v198.028l-184.06 123.13l-148.599-99.403zM84.916 428.943l106.226 71.05l-106.226 71.054zm366.651 443.278L118.908 650.464l148.599-99.377l184.06 123.105zm42.464-271.783L343.86 499.993L494.031 399.55L644.19 499.993zm42.453 271.783V674.192l184.069-123.105l148.588 99.377zm366.648-301.174l-106.213-71.054l106.213-71.05z"/>',
    "deviantart":'<path fill="currentColor" d="m362.844 293.375l4.906 339.219H161.281c23.034-97.986 69.423-176.972 203-196.656l-1.438-44.25C69.007 430.08 6.665 591.516 2.53 706.626l473.375-.281V423.189c142.178-3.648 319.852 1.934 368.688 207.719l-210.313-1.719V484.908c-37.66-10.995-70.11-11.183-106.031-8.687l-1.75 229.531l469.313-.094c4.511-229.461-199.386-333.433-519.906-328.5v-83.781H362.843z"/>',
    "diigo":'<path fill="currentColor" d="M250.64 393.24c112.01-1.75 173.43-61.25 230.68-116.99v672.38H368.5c-1.02-27.07-1.59-54.49-2.01-82.03V678.46c.76-71.14 1.91-142.32 2.17-212.16c-15.86 4.21-33.75 12.58-53.92 17.89c-60.61 15.99-140.37 7.62-186.5-19.38c.15 33.14.37 66.55.63 100.07v351.6c-.19 10.72-.31 21.48-.54 32.17H15.07V274.5C72.96 335.79 137.09 395 250.64 393.24m176.8-196.08c40.09 0 72.57-33.26 72.57-74.27s-32.48-74.26-72.57-74.26c-40.08 0-72.58 33.24-72.58 74.26s32.5 74.27 72.58 74.27m-354.85 0c40.08 0 72.56-33.26 72.56-74.27s-32.48-74.26-72.56-74.26C32.5 48.63 0 81.87 0 122.89s32.5 74.27 72.59 74.27"/>',
    "discover":'<path fill="currentColor" d="M45.938 186c-25.69 0-46.532 20.814-46.532 46.5v535c0 25.689 20.844 46.5 46.532 46.5h906.968c25.69 0 46.5-20.814 46.5-46.5v-535c0-25.689-20.812-46.5-46.5-46.5zm479.937 176.781c.678-.017 1.349 0 2.031 0c43.67 0 79.063 35.387 79.063 79.063c0 43.675-35.394 79.094-79.063 79.094s-79.062-35.419-79.062-79.094c0-42.993 34.305-77.98 77.031-79.063m-249.563 3.469c15.675 0 30.042 5.38 42.032 15.875L303.75 401.25c-7.27-8.152-14.146-11.594-22.5-11.594c-12.028 0-20.781 6.835-20.781 15.844c0 7.726 4.915 11.836 21.656 18.031c31.73 11.607 41.125 21.899 41.125 44.625c0 27.695-20.323 46.969-49.313 46.969c-21.221 0-36.68-8.381-49.53-27.25l18.03-17.344c6.42 12.425 17.134 19.063 30.438 19.063c12.442 0 21.656-8.567 21.656-20.156c0-6.008-2.769-11.17-8.344-14.813c-2.806-1.724-8.363-4.304-19.28-8.156c-26.2-9.435-35.188-19.51-35.188-39.219c0-23.412 19.289-41 44.594-41zm131.22 0c12 0 22.094 2.565 34.343 8.75v32.688c-11.603-11.298-21.69-16.032-35-16.032c-26.162 0-46.719 21.595-46.719 48.938c0 28.845 19.936 49.125 48.031 49.125c12.648 0 22.533-4.479 33.688-15.594v32.688c-12.686 5.948-22.988 8.312-35 8.312c-42.478 0-75.469-32.48-75.469-74.344c0-41.413 33.863-74.531 76.125-74.531zm-352.876 2.344h41.25c45.578 0 77.344 29.158 77.344 71.031c0 20.879-9.724 41.072-26.156 54.469c-13.833 11.308-29.586 16.375-51.406 16.375H54.656zm131.375 0h26.75v141.875h-26.75zm409.313 0h30.406l38.063 95.969l38.562-95.97h30.188l-61.657 146.532h-15zm148.843 0h79.063v24.031h-51.219v31.5h49.344v24.031h-49.344v38.281h51.219v24.032h-79.063zm98.844 0h41.438c32.246 0 50.75 15.312 50.75 41.875c0 21.723-11.597 35.976-32.657 40.219l45.125 59.78h-34.406l-38.687-57h-3.657v57h-27.906V368.595zm28.094 22.344v43.187h8.094c17.681 0 27.062-7.7 27.062-22.031c0-13.874-9.382-21.156-26.625-21.156zM82.75 392.5v94.063h7.531c18.18 0 29.693-3.437 38.531-11.313c9.737-8.541 15.563-22.186 15.563-35.844c0-13.625-5.826-26.86-15.563-35.406c-9.29-8.3-20.352-11.5-38.53-11.5zm904.469 134.188s.28 152.372-.344 232.468c-.055 7.01-2.621 18.107-10.594 27.344s-20.373 13.558-27.781 13.531s-724.094 0-724.094 0C716.298 715.174 987.22 526.688 987.22 526.688z"/>',
    "facebook":'<path fill="currentColor" d="M124.074 1000V530.771H0V361.826h124.074V217.525C124.074 104.132 197.365 0 366.243 0C434.619 0 485.18 6.555 485.18 6.555l-3.984 157.766s-51.564-.502-107.833-.502c-60.9 0-70.657 28.065-70.657 74.646v123.361h183.331l-7.977 168.945H302.706V1000z"/>',
    "facebook-rect":'<path fill="currentColor" d="M182.594 0C81.445 0 0 81.445 0 182.594v634.813c0 101.149 81.445 182.594 182.594 182.594h344.063V609.063H423.282v-140.75h103.375v-120.25c0-94.475 61.079-181.219 201.781-181.219c56.968 0 99.094 5.469 99.094 5.469l-3.313 131.438s-42.963-.406-89.844-.406c-50.739 0-58.875 23.378-58.875 62.188v102.781h152.75l-6.656 140.75H675.5v390.938h141.906c101.149 0 182.594-81.445 182.594-182.594V182.595C1000 81.446 918.555.001 817.406.001H182.593z"/>',
    "friendfeed":'<path fill="currentColor" d="M716.905 0c-47.213.083-98.225 8.416-150.064 36.045c-58.326-20.952-119.915-38.638-205.54-32.291C167.643 21.787 93.724 119.642 82.355 259.201C41.518 263.853 2.927 274.622.126 382.513c-2.68 122.192 35.56 142.107 76.346 155.634l2.942 358.202c5.763 42.297-2.143 89.433 93.963 99.845l143.87 2.942c52.66 6.862 89.196-26.6 108.637-102.786c6.832 61.932 34.051 95.866 85.17 96.903h167.367c37.832-10.083 79.332-13.433 85.138-82.229l2.942-372.877l44.025 2.91c35.955-5.207 71.771-11.146 96.903-70.464V359.014c-7.266-45.549-19.589-40.149-32.291-49.907c34.192-7.566 69.505-11.149 82.198-93.963l-2.91-91.021c1.195-57.109-30.624-89.554-110.358-110.703C807.533 8.086 764.12-.087 716.908-.003zM383.578 87.391c23.019-.257 54.745 3.486 121.278 24.812l2.066 107.98c-34.62-12.135-64.936-26.136-89.269-22.842c-75.301-3.536-82.365 34.895-87.204 74.719v76.847l193.088 2.066c4.647-101.809-22.732-231.625 145.934-254.071c73.63-9.793 141.276-.385 192.493 21.527l-2.066 114.207c-26.481-21.181-62.23-28.468-99.657-33.23c-104.108-14.637-80.128 78.591-85.138 147.405h139.114v101.753H675.103l2.066 460.926l-153.632-4.161V448.565l-199.315 4.13v452.634H170.59l4.13-452.634H99.97L97.904 338.52l80.977 4.161l-2.066-31.164c3.628-74.406-2.875-202.671 156.573-222.656c19.648 1.352 32.285-1.271 50.188-1.471z"/>',
    "friendfeed-rect":'<path fill="currentColor" d="M195.844.406C88.147.406 0 88.522 0 196.219v607.719c0 107.697 88.147 195.813 195.844 195.813h607.719c107.697 0 195.813-88.116 195.813-195.813V196.219C999.376 88.522 911.261.406 803.563.406zM674 138.156c34.547-.061 66.298 5.91 93.031 9.813c58.344 15.475 81.624 39.211 80.75 81l2.156 66.625c-9.288 60.597-35.137 63.214-60.156 68.75c9.294 7.14 18.308 3.202 23.625 36.531V482.5c-18.391 43.405-44.597 47.752-70.906 51.563l-32.219-2.125l-2.156 272.844c-4.249 50.34-34.629 52.779-62.313 60.156H523.343c-37.405-.76-57.282-25.588-62.281-70.906c-14.226 55.748-40.966 80.24-79.5 75.219l-105.281-2.156c-70.323-7.619-64.533-42.112-68.75-73.063l-2.156-262.094c-29.845-9.898-57.836-24.463-55.875-113.875c2.049-78.947 30.275-86.846 60.156-90.25c8.319-102.12 62.419-173.742 204.125-186.938c62.655-4.644 107.727 8.325 150.406 23.656c37.933-20.217 75.266-26.314 109.813-26.375m-243.906 63.938c-13.101.147-22.342 2.083-36.719 1.094c-116.673 14.624-111.939 108.461-114.594 162.906l1.531 22.813l-59.25-3.063l1.5 83.563h54.719l-3.063 331.219h112.438V469.407l145.844-3.031v334.25l112.438 3.031l-1.531-337.281h101.781v-74.438H643.407c3.666-50.353-13.898-118.586 62.281-107.875c27.387 3.484 53.56 8.814 72.938 24.313l1.531-83.563c-37.477-16.034-86.967-22.916-140.844-15.75C515.895 225.487 535.9 320.472 532.5 394.969l-141.281-1.531V337.25c3.541-29.141 8.712-57.307 63.813-54.719c17.805-2.411 39.98 7.839 65.313 16.719l-1.5-79c-48.684-15.605-71.906-18.345-88.75-18.156z"/>',
    "github":'<path fill="currentColor" d="M1078.94-.985c-33.192-.491-110.295 10.777-239.027 96.936c-70.161-17.535-144.812-26.188-219.591-26.188c-82.278 0-165.425 10.448-242.965 31.719C192.534-24.605 110.955 1.234 110.955 1.234c-53.258 133.183-20.347 231.788-10.344 256.277C38.014 325.069-.2 411.338-.2 517.07c0 79.822 9.085 151.416 31.281 213.653c1.231 4.803.832 3.732 2.906 7.844c4.89 12.884 10.327 25.39 16.438 37.468c2.094 4.346 4 7.563 4 7.563c62.395 116.307 185.396 191.438 404.244 215.028l330.995.375c233.392-23.144 345.386-98.499 396.994-215.591l3.281-7.625c4.89-11.828 9.153-24.135 20.813-65.562s16.875-113.172 16.875-193.185c0-114.755-43.1-206.577-113.092-276.434c12.231-39.48 28.57-127.158-16.313-239.402c0 0-6.293-1.995-19.281-2.188zM818.1 420.133c53.893-.117 100.057 9.136 134.717 45.499v.031c43.369 45.541 68.749 100.525 68.749 159.778c0 276.658-177.932 284.183-397.4 284.183c-219.506 0-397.4-38.336-397.4-284.183c0-58.861 25.009-113.516 67.843-158.872c71.451-75.59 192.365-35.562 329.558-35.562c70.423-.011 136.564-10.75 193.935-10.875zm-408.807 81.468c-45.666 0-82.687 61.741-82.687 137.936c0 76.206 37.019 137.967 82.687 137.967c45.666 0 82.687-61.761 82.687-137.967c0-76.184-37.019-137.881-82.687-137.936m443.649 0c-45.666 0-82.687 61.741-82.687 137.936c0 76.206 37.019 137.967 82.687 137.967c45.666 0 82.687-61.761 82.687-137.967c0-76.184-37.019-137.881-82.687-137.936"/>',
    "github-text":'<path fill="currentColor" d="M526.961 0c-21.136 0-39.171 8.011-54.113 24.045c-14.939 16.037-22.39 35.359-22.39 57.954c0 21.866 7.448 40.798 22.39 56.831c14.942 16.037 32.978 24.073 54.113 24.073c20.409 0 38.08-8.036 53.022-24.073c14.942-16.033 22.419-34.965 22.419-56.831c0-22.596-7.476-41.917-22.419-57.954C565.044 8.012 547.37 0 526.961 0m163.903 156.345c2.183 12.391 3.278 28.772 3.278 49.181v42.622h-53.55v106.041c15.304-2.186 27.684-3.246 37.159-3.246l16.395 1.063v2.186h-1.091v195.69c0 61.228 7.678 106.058 22.982 134.486c20.409 37.896 57.21 56.831 110.413 56.831c37.899 0 70.331-7.274 97.299-21.859V610.018c-18.227 12.394-38.64 18.578-61.232 18.578c-32.067 0-48.086-24.42-48.086-73.254v-203.34h46.995q13.124.001 27.886 1.091c9.84.73 16.561 1.091 20.204 1.091V248.143h-95.081v-44.809c0-17.49 1.095-33.144 3.278-46.995H690.877l-.011.004zm-500.638 86.34c-49.56 0-92.183 16.413-127.899 49.209c-37.166 34.983-55.768 78.7-55.768 131.177c0 34.983 10.194 67.784 30.6 98.391c18.22 28.421 39.005 46.658 62.327 54.677v2.186c-23.322 10.194-34.972 35.72-34.972 76.532c0 31.344 12.38 54.641 37.159 69.945v2.186C33.897 749.58.005 791.483.005 852.701c0 53.21 22.599 92.222 67.791 117c35.706 19.68 81.252 29.509 136.641 29.509c134.83 0 202.249-56.487 202.249-169.459c0-70.693-52.119-114.056-156.345-130.086c-24.048-3.643-42.254-12.377-54.645-26.228c-9.475-9.471-14.209-18.971-14.209-28.446c0-26.965 14.57-42.619 43.714-46.995c44.458-6.552 80.727-27.145 108.79-61.763c28.059-34.622 42.091-75.26 42.091-121.904c0-14.577-2.913-30.235-8.741-46.995c18.953-4.376 33.509-8.38 43.714-12.022V242.681c-45.921 17.493-86.361 26.259-121.341 26.259c-30.614-17.49-63.772-26.259-99.486-26.259v.004zm274.44 5.464c1.456 13.117 2.186 36.819 2.186 71.068v340.012c0 34.987-.73 59.747-2.186 74.317h123.527c-1.46-16.76-2.186-43.732-2.186-80.904V317.03c0-32.067.726-55.035 2.186-68.882H464.666zm-270.068 95.113c21.866 0 38.976 8.408 51.367 25.168c10.201 15.311 15.3 33.509 15.3 54.645c0 51.017-22.209 76.532-66.667 76.532c-45.911 0-68.882-25.15-68.882-75.441c0-53.929 22.971-80.904 68.882-80.904m7.65 444.962c61.225 0 91.836 18.567 91.836 55.736c0 39.359-28.063 59.046-84.182 59.046c-64.134 0-96.204-18.967-96.204-56.863c0-38.629 29.519-57.922 88.554-57.922z"/>',
    "googleplus-rect":'<path fill="currentColor" d="M92.094 0C41.226 0 0 26.304 0 92.094v78.969c3.632-3.5 7.837-7.39 11.719-10.719C94.522 91.805 185.36 80.346 261.532 78.938h307.031L487 144.751h-89c35.658 30.921 88.844 86.196 88.844 184.656c0 107.855-56.831 158.961-117.844 207c-18.882 18.908-40.656 39.382-40.656 71.5c0 32.039 21.771 49.543 37.719 62.688l52.344 40.719c63.868 53.961 147.375 116.444 147.375 217.063c0 26.947-2.924 51.151-11.688 71.625h353.813c50.869 0 92.094-28.069 92.094-78.938V340.501H815.782v185.813h-78.938V340.501H552.625v-77.344h184.219V78.938h78.938v184.219h184.219V92.094C1000.001 26.305 958.776 0 907.907 0zM1000 263.156V340.5h.656v-77.344zM187.063 128.25c-37.803 0-78.429 18.936-101.719 48.094c-24.697 30.632-28.5 69.919-28.5 107.906c0 97.618 51.285 242.063 176.219 242.063c36.289 0 73.512-9.219 97.406-36.469c27.263-31.224 37.938-72.331 37.938-93.344c0-104.987-60.844-268.25-181.344-268.25M0 526.313v258.219c65.789-52.632 179.88-43.174 235.406-46.938c-20.368-26.276-43.594-53.949-43.594-99.094c0-24.789 7.281-39.393 14.531-56.906c-16.013 1.434-31.934 2.938-46.5 2.938c-65.908 0-120.028-25.114-159.844-58.219zm251.438 252.093c-13.132 0-91.607 2.941-152.594 23.375c-20.987 7.671-68.87 25.875-98.844 66.625v52.656C0 971.93 41.225 1000 92.094 1000h376.969c2.171-7.395 6.252-11.878 6.094-26.313c-.816-74.487-79.959-115.018-190.406-193.781c-11.579-1.5-18.865-1.5-33.313-1.5"/>',
    "houzz":'<path fill="currentColor" d="M288.672 0v333.329L0 500.012V166.635zM.006 499.988v333.323L288.678 666.58L.036 499.987zM288.672 666.58v333.377l288.666-166.671V499.987zm0-333.227l288.672 166.635V166.635z"/>',
    "icq":'<path fill="currentColor" d="M354.544 27.284c-31.01-7.051-56.394-4.231-83.173 9.872c-29.614 18.333-46.537 46.537-53.588 86.008c-7.051 40.881 0 80.367 21.153 119.838l9.871 14.088l-33.845-11.282c-42.292-9.871-81.777-4.231-121.248 16.922c-40.881 21.153-66.265 50.753-83.187 91.649c-15.512 39.485-14.088 77.547 5.641 112.787c8.461 15.512 21.153 28.19 32.42 36.651c16.922 16.923 38.075 26.794 63.445 31.01l9.871 2.82l-21.153 18.333c-19.743 16.922-31.01 38.061-38.061 59.214c-7.051 25.384-4.231 50.753 9.871 74.726c7.051 11.267 14.088 21.139 25.369 28.189c11.282 9.871 25.384 16.923 42.292 18.318c31.024 7.051 63.445 7.051 98.685-4.245l-25.384 42.292l-7.051 18.347c-14.088 45.112-9.871 84.597 11.282 125.465c9.871 15.511 21.153 29.628 32.435 42.291c16.922 15.512 38.061 25.383 63.445 32.435c42.292 11.267 84.598 9.871 125.479-11.267c39.486-21.168 64.855-54.998 77.547-97.303l1.41-8.462c26.779 29.629 52.163 49.357 80.367 56.408c28.19 8.461 56.394 7.051 81.777-9.871c23.973-14.102 40.881-38.075 49.343-67.675c7.051-31.025 7.051-62.035 0-97.275c14.088 9.871 28.19 16.894 45.112 21.139c45.112 14.102 84.583 12.678 124.069-8.461c38.075-21.139 63.445-54.998 76.136-98.7c11.282-42.292 8.461-83.159-12.692-122.644l-7.051-11.282c-11.282-16.922-22.549-33.845-39.471-45.112c-14.087-9.871-31.01-16.922-46.522-23.973c43.716-21.153 74.726-50.753 88.828-88.828c9.871-31.024 7.051-56.393-7.051-81.777v-2.82c-16.908-28.204-39.471-46.537-74.712-56.393c-21.153-4.231-45.126-4.231-66.265-2.82l-46.522 9.871l15.512-35.24c14.102-45.112 9.871-88.828-14.102-129.709l-4.231-7.051c-21.153-35.255-52.163-56.393-94.455-67.675c-43.702-11.281-84.583-7.051-124.069 14.102c-39.471 21.139-67.675 53.573-80.353 95.879v2.82l-1.41 9.872l-5.641-9.872l-14.102-25.384c-22.578-35.197-50.781-56.35-86.022-66.222zm359.529 84.597c7.051 14.088 11.281 31.01 11.281 49.343l-4.231 31.01c-5.641 15.512-12.692 29.614-21.153 42.292L573.081 389.634c-12.692-9.872-26.794-16.923-40.896-23.974l-14.088-159.309l-1.41-56.393l1.41-14.088c7.051-28.189 26.779-49.342 52.163-63.445c25.369-14.088 53.573-16.922 80.353-7.051c29.614 7.037 50.767 22.549 63.459 46.508zm-437.075 28.204c1.41-25.384 11.282-42.292 28.19-52.163c8.461-7.051 21.153-7.051 32.42-1.41c25.384 7.051 46.522 25.384 63.445 60.624c7.051 14.102 14.102 28.189 16.922 45.112l28.19 166.375l-21.153 7.051l-125.479-136.761l-7.051-14.088c-11.253-25.398-18.304-49.357-15.483-74.741zm-76.136 164.95l38.075 15.512l124.069 90.238c-11.282 14.088-18.318 28.189-22.549 42.292l-203.026 25.384l-14.102-2.82c-25.384-4.231-42.292-15.512-53.573-36.651c-11.282-21.153-11.282-42.292-2.82-63.445c9.871-26.794 28.19-47.947 54.983-59.214c25.369-14.116 50.738-18.347 78.942-11.296zm133.94 229.805l9.871 28.204l4.231 9.871c-21.153 18.318-36.665 35.241-53.573 50.753l-42.306 35.255l-11.282 7.051c-26.794 12.678-49.343 16.923-73.316 14.088c-21.153-1.41-38.075-8.461-45.126-21.139l-4.231-15.512l2.82-12.677c7.051-21.167 25.384-42.292 57.818-57.818c16.908-7.051 31.01-14.102 42.292-16.922zm439.881-228.395c25.384-11.281 49.343-14.102 74.726-11.281c23.973 2.82 40.896 14.088 49.342 31.01l5.641 18.318l-2.82 14.102c-9.871 25.384-31.024 45.112-66.265 60.624c-14.087 7.051-29.6 9.872-46.522 12.677l-173.411 21.153l-2.82-9.871l-2.82-4.216l5.641-7.051l142.401-118.428zm-359.514 133.94c16.908-15.512 39.471-25.369 66.265-25.369c25.384 0 49.343 9.871 64.855 25.369c19.743 19.743 28.204 40.896 28.204 66.265s-8.461 49.343-28.204 66.265c-18.333 18.319-39.471 28.19-64.855 28.19c-26.794 0-49.357-9.871-66.265-28.19c-18.333-16.922-28.19-40.896-28.19-66.265s9.857-46.522 28.19-66.265m32.42 204.436c2.82 47.947 7.051 98.699 14.102 153.683v56.408l-1.41 14.102c-9.871 25.355-26.794 46.508-52.163 57.789c-25.369 14.102-52.163 16.923-77.547 9.872c-28.19-7.022-49.343-23.945-60.624-46.508c-7.051-14.102-12.692-31.025-12.692-47.947l7.051-32.405c2.82-14.102 9.871-28.219 21.153-39.485l118.428-141.006c14.102 7.036 28.19 14.088 43.702 15.498zm108.571-18.332l59.214 60.624l36.651 45.112l7.051 14.102c12.692 23.974 19.743 46.537 16.922 73.331c-2.82 25.383-11.282 43.701-28.19 50.752c-9.871 7.051-21.153 7.051-32.42 5.627c-26.794-7.022-47.947-28.19-63.445-62.035l-16.922-46.508l-21.153-122.673l23.973-8.461l18.318-9.871zm63.445-87.418l159.324-14.088l56.394-1.411l14.102 1.411c21.153 7.051 40.896 19.743 52.163 40.881c14.088 15.512 21.139 36.651 21.139 60.624l-4.216 28.189c-7.051 28.219-23.973 49.357-47.947 60.624c-25.384 14.102-50.753 16.922-78.957 7.051c-16.922-4.216-28.19-11.267-38.075-22.535L605.49 577.144l9.871-21.139l4.245-16.937z"/>',
    "instagram":'<path fill="currentColor" d="M749.359 1.095H250.643c-137.4 0-249.186 111.41-249.186 248.348v501.11c0 136.929 111.786 248.351 249.186 248.351h498.716c137.4 0 249.183-111.423 249.183-248.351V249.442c0-136.937-111.783-248.346-249.183-248.346zm156.895 749.456c0 86.214-70.38 156.368-156.894 156.368H250.644c-86.514 0-156.895-70.154-156.895-156.368V426.903h197.146c-14.393 30.443-22.488 64.404-22.488 100.218c0 130.083 106.18 235.897 236.696 235.897c130.513 0 236.695-105.814 236.695-235.897c0-35.814-8.094-69.776-22.49-100.218h186.945v323.648zM406.845 527.123c0-53.99 44.077-97.923 98.259-97.923c54.184 0 98.258 43.934 98.258 97.923c0 54.001-44.074 97.926-98.258 97.926c-54.181-.001-98.259-43.925-98.259-97.926m499.409-192.199H642.08c-38.701-27.468-85.954-43.695-136.976-43.695s-98.281 16.227-136.973 43.695H93.749v-85.481c0-86.223 70.381-156.367 156.895-156.367H749.36c86.514 0 156.894 70.144 156.894 156.367zM806.248 148.861h-85.55c-15.232 0-27.687 12.418-27.687 27.594v85.266c0 15.175 12.454 27.594 27.687 27.594h85.55c15.231 0 27.687-12.419 27.687-27.594v-85.266c0-15.176-12.455-27.594-27.687-27.594"/>',
    "instagram-filled":'<path fill="currentColor" d="M603.575 510.854c0-54.044-44.172-98.02-98.463-98.02c-54.285 0-98.455 43.977-98.455 98.02c0 54.058 44.17 98.026 98.455 98.026c54.291 0 98.463-43.968 98.463-98.026M749.855.602H250.142C112.465.602.456 112.124.456 249.199v69.266h367.413c38.769-27.496 86.118-43.739 137.243-43.739s98.48 16.243 137.251 43.739H999.54v-69.266C999.54 112.123 887.531.602 749.855.602m101.12 236.435c0 15.191-12.48 27.622-27.743 27.622h-85.72c-15.261 0-27.743-12.431-27.743-27.622v-85.352c0-15.19 12.481-27.622 27.743-27.622h85.72c15.262 0 27.743 12.43 27.743 27.622zM742.287 510.854c0 130.215-106.402 236.133-237.176 236.133c-130.778 0-237.17-105.918-237.17-236.133c0-35.848 8.11-69.845 22.532-100.318H.456v340.263c0 137.085 112.009 248.597 249.686 248.597h499.714c137.676 0 249.685-111.513 249.685-248.597V410.536H719.755c14.422 30.473 22.532 64.47 22.532 100.318"/>',
    "jabber":'<path fill="currentColor" d="M224.968 0c-4.449-.03-8.858.158-13.121.719V.688c-18.133 2.383-36.574 2.121-54.798 4.937c-34.916 5.391-68.366 16.252-101.066 29.336c-11.907 4.762-23.639 10.067-35.178 15.652c-4.294 2.078-10.856 3.726-14.309 7.061c-1.796 1.738-1.594 5.83-1.812 8.154c-.607 6.539-1.429 13.053-2.062 19.589c-2.442 25.217-4.174 51.214-.563 76.417c1.169 8.144 3.706 22.901 13.715 24.462c15.392 2.408 27.343-16.533 34.896-26.992c17.642-24.425 37.734-47.157 66.669-58.078c34.057-12.857 73.039-15.899 109.063-18.37c101.1-6.929 211.602 11.542 286.076 86.007c10.591 10.59 20.622 21.824 29.148 34.178c33.346 48.282 50.96 103.199 59.452 160.861c3.993 27.074 6.738 54.455.563 81.415c-10.717 46.775-38.817 87.493-71.105 122.154c-22.1 23.723-46.972 45.178-72.855 64.67c-14.45 10.879-30.651 20.097-44.238 31.991c-15.648 13.698-28.66 30.833-38.552 49.111c-26.622 49.213-33.761 108.429-25.931 163.36c1.605 11.26 3.041 22.573 5.592 33.679c.763 3.322.913 9.032 3.062 11.747c4.022 5.078 14.973 7.888 21.119 8.186c6.41.31 10.675-4.625 12.247-10.372c2.689-9.848 1.09-20.741-.469-30.647c-3.192-20.291-6.122-40.73-5.218-61.327c1.668-38.032 16.376-73.468 42.738-101.035c26.809-28.034 63.037-44.216 93.505-67.7c61.404-47.328 116.4-111.175 137.524-187.167c7.55-27.173 10.04-55.97 6.592-83.945c-4.357-35.385-17.3-70.741-29.492-104.065C599.005 182.403 535.251 85.831 434.029 39.369C393.862 20.927 351.872 12.53 308.376 6.472c-20.407-2.84-40.799-4.598-61.327-5.498c-7.135-.312-14.673-.919-22.088-.969zm-52.641 421.507c-10.102.082-20.142 2.657-28.867 7.967c-12.828 7.813-19.97 21.456-23.993 35.459c-7.986 27.806-4.304 59.519 1.281 87.445c1.785 8.931 3.618 17.882 6.123 26.649c.905 3.163 1.317 7.773 3.374 10.403c3.59 4.6 12.371 5.934 17.745 5.03c16.125-2.71 12.14-29.262 11.591-40.552c-1.287-26.445-9.08-52.112-1.531-78.416c3.197-11.13 9.346-23.657 20.588-28.492c10.147-4.369 20.412.961 24.431 10.872c6.506 16.049 4.855 34.6 8.935 51.299c1.387 5.676 2.938 11.298 5.498 16.589c.742 1.529 1.756 4.692 3.905 4.499c5.225-.478 12.64-9.2 16.558-12.435c12.652-10.456 25.671-21.629 40.239-29.367c6.854-3.64 18.118-9.534 25.118-3.343c1.785 1.58 3.221 3.438 4.499 5.436c8.229 12.889.07 22.953-5.28 35.209c-6.925 15.864-14.022 31.561-24.056 45.737c-8.627 12.191-20.413 22.892-27.305 36.209c-4.437 8.575-6.959 17.972-5.124 27.648c1.488 7.825 7.546 12.55 15.527 10.716c5.974-1.374 10.027-6.521 13.434-11.247c6.07-8.429 11.712-17.155 17.745-25.618c11.673-16.366 25.455-31.674 35.271-49.267c11.832-21.21 17.54-45.197 17.526-69.356c-.008-19.742-7.182-49.94-30.18-54.079c-4.328-.775-9.26-.344-13.559.406a59.7 59.7 0 0 0-15.59 5.03c-13.075 6.325-24.4 15.783-34.115 26.524c-6.052 6.683-11.474 15.061-18.651 20.557c-.004-14.574-2.719-30.456-11.185-42.676c-11.147-16.084-30.669-24.993-49.955-24.837zM56.89 657.847c-1.113.04-2.252.143-3.437.313c-19.704 2.835-25.449 18.659-21.807 36.49c4.668 22.831 22.634 40.24 40.427 53.954c45.587 35.138 104.458 54.124 161.861 51.611c22.401-.981 46.83-4.679 67.387-14.184c11.023-5.094 22.541-23.449 7.529-31.304c-2.057-1.077-4.738-1.717-7.029-2.093c-6.556-1.073-13.906 1.715-20.119 3.499c-10.956 3.146-21.899 6.344-33.179 8.154c-33.697 5.412-70.136-2.279-98.036-22.244c-22.566-16.144-40.795-37.431-58.015-58.952c-9.882-12.341-18.889-25.84-35.584-25.243zm-20.713 111.25c-2.983.031-5.966.344-8.342.656c-5.019.662-10.76.47-15.059 3.624c-8.965 6.581.906 19.837 5.623 25.837c13.678 17.38 31.669 31.802 49.642 44.519c50.108 35.443 106.88 55.893 167.922 60.077c17.55 1.203 35.44-.046 52.798-2.937c6.347-1.06 14.222-1.541 20.088-4.28c16.598-7.763 11.815-39.109-2.999-46.081c-6.469-3.041-16.866.963-23.649 1.905c-16.043 2.237-32.133 2.455-48.267 2.062c-42.285-1.035-87.309-14.063-124.153-34.615c-14.484-8.085-27.803-17.921-40.239-28.867c-7.562-6.656-15.311-17.537-25.118-20.9c-2.283-.784-5.265-1.031-8.248-1zm3.093 94.599c-.458.001-.912.011-1.375.063c-7.147.8-12.544 4.538-17.245 9.841c-3.239 3.653-6.293 6.951-8.154 11.56c-10.256 25.37 17.076 49.314 35.959 61.296c21.014 13.329 43.966 23.199 67.356 31.522c52.755 18.776 114.637 30.813 169.422 14.027c16.285-4.985 32.367-14.11 44.207-26.431c6.669-6.941 11.265-17.456-.969-21.9c-7.248-2.631-16.045 4.07-22.619 6.717c-15.519 6.258-31.609 11.013-48.267 13.027c-21.94 2.66-41.712-2.175-62.857-7.467c-18.062-4.516-36.304-9.017-53.766-15.527c-14.898-5.554-29.702-12.145-42.738-21.369c-3.896-2.756-8.555-5.221-11.747-8.811c-10.436-11.742-18.302-25.479-29.492-36.646c-4.533-4.525-10.845-9.925-17.714-9.903z"/>',
    "lastfm":'<path fill="currentColor" d="m438.782 715.968l-36.475-99.147s-59.271 66.104-148.153 66.104c-78.648 0-134.487-68.383-134.487-177.795c0-140.18 70.653-190.326 140.184-190.326c100.286 0 132.194 64.96 159.553 148.161l36.475 113.97c36.462 110.543 104.845 199.438 302.008 199.438c141.333 0 237.061-43.304 237.061-157.269c0-92.314-52.433-140.184-150.449-162.98l-72.937-15.954c-50.141-11.396-64.955-31.912-64.955-66.104c0-38.75 30.764-61.545 80.918-61.545c54.704 0 84.332 20.517 88.895 69.522l113.966-13.679c-9.117-102.57-79.774-144.738-196.023-144.738c-102.566 0-202.861 38.75-202.861 162.976c0 77.499 37.61 126.51 132.194 149.301l77.508 18.229c58.122 13.679 77.491 37.61 77.491 70.666c0 42.16-41.025 59.258-118.524 59.258c-115.11 0-162.967-60.402-190.325-143.603l-37.61-113.961C484.378 278.331 408.01 223.627 256.43 223.627C88.9 223.627.001 329.616.001 509.685c0 173.236 88.9 266.685 248.461 266.685c128.767 0 190.321-60.402 190.321-60.402z"/>',
    "lastfm-rect":'<path fill="currentColor" d="M196.049.188C88.327.188.157 88.328.157 196.049v607.863c0 107.721 88.171 195.861 195.892 195.861h607.863c107.722 0 195.861-88.14 195.861-195.861V196.049C999.773 88.328 911.634.188 803.912.188zm88.713 253.013c135.353 0 203.53 48.874 246.264 181.175l33.592 101.743c24.429 74.295 67.169 128.241 169.957 128.241c69.203 0 105.836-15.287 105.836-52.934c0-29.518-17.283-50.874-69.183-63.09l-69.214-16.28c-84.459-20.352-118.055-64.099-118.055-133.304c0-110.928 89.558-145.553 181.143-145.553c103.805 0 166.909 37.651 175.051 129.241l-101.774 12.218c-4.075-43.76-30.522-62.059-79.369-62.059c-44.785 0-72.276 20.332-72.276 54.934c0 30.531 13.254 48.852 58.027 59.027l65.12 14.249c87.523 20.356 134.334 63.121 134.334 145.553c0 101.766-85.47 140.428-211.673 140.428c-176.056 0-237.11-79.372-269.669-178.081l-32.592-101.774c-24.429-74.295-52.908-132.304-142.459-132.304c-62.088 0-125.179 44.783-125.179 169.957c0 97.699 49.856 158.739 120.085 158.739c79.367 0 132.304-59.027 132.304-59.027l32.561 88.557s-54.974 53.934-169.957 53.934c-142.481 0-221.859-83.448-221.859-238.139c0-160.793 79.388-255.451 228.984-255.451z"/>',
    "linkedin":'<path fill="currentColor" d="M237.485 1000V325.301H13.229V1000zM125.386 233.127c78.202 0 126.879-51.809 126.879-116.553C250.808 50.37 203.591-.001 126.87-.001C50.161-.001-.002 50.371-.002 116.574c0 64.747 48.665 116.553 123.924 116.553h1.457zM361.61 1000h224.256V623.215c0-20.165 1.457-40.309 7.379-54.724c16.212-40.289 53.111-82.017 115.06-82.017c81.149 0 113.613 61.872 113.613 152.572v360.949h224.242V613.129c0-207.241-110.636-303.668-258.183-303.668c-120.977 0-174.094 67.622-203.603 113.679h1.497v-97.853H361.615c2.943 63.31 0 674.699 0 674.699z"/>',
    "linkedin-rect":'<path fill="currentColor" d="M196.064.25C88.347.25.187 88.408.187 196.127v607.841c0 107.717 88.158 195.845 195.877 195.845h607.841c107.718 0 195.845-88.127 195.845-195.845V196.127C999.75 88.41 911.623.25 803.905.25zm49.266 164.948c51.648 0 83.461 33.906 84.443 78.475c0 43.585-32.797 78.444-85.442 78.444h-.969c-50.665 0-83.412-34.857-83.412-78.444c0-44.568 33.738-78.475 85.379-78.475zm445.08 208.31c99.329 0 173.79 64.922 173.79 204.436v260.449H713.247V595.406c0-61.06-21.847-102.718-76.476-102.718c-41.704 0-66.562 28.078-77.476 55.202c-3.987 9.704-4.967 23.257-4.967 36.832v253.671H403.375s1.981-411.613 0-454.233h150.984v64.324c20.06-30.95 55.942-74.977 136.051-74.977zm-521.556 10.685h150.953v454.202H168.854z"/>',
    "mastercard":'<path fill="currentColor" d="M46.531 195C20.841 195 0 215.83 0 241.531V776.47C0 802.166 20.844 823 46.531 823H953.47c25.69 0 46.531-20.83 46.531-46.531V241.53c0-25.697-20.844-46.531-46.531-46.531H46.53zm281.375 75.594c57.081 0 109.505 19.859 150.656 53.031c-34.622 30.979-60.446 71.68-73.187 117.813h19.969c11.76-38.058 33.123-71.942 61.281-98.813c28.158 26.871 49.52 60.755 61.281 98.813h20.282c-12.918-45.89-38.778-86.348-73.375-117.125c41.23-33.593 93.86-53.72 151.187-53.72c132.291 0 239.531 107.252 239.531 239.563S778.291 749.719 646 749.719c-58.28 0-111.72-20.802-153.25-55.407c37.15-32.32 64.538-75.647 77.063-124.937H550.28c-11.121 41.37-33.505 78.165-63.656 106.938c-30.15-28.773-52.503-65.568-63.625-106.938h-18.594c12.598 48.576 39.628 91.297 76.219 123.313c-41.454 34.176-94.684 54.718-152.719 54.718c-132.29 0-239.53-106.736-239.53-238.406s107.24-238.406 239.531-238.406M628.031 439c-28.781 0-56.781 25.162-56.781 72.406c0 31.328 15.124 52 44.875 52c8.412.003 21.813-3.437 21.813-3.437l4.437-27.375s-12.23 6.187-20.625 6.187c-17.685 0-24.781-13.601-24.781-28.218c0-29.653 15.216-45.97 32.156-45.97c12.703 0 22.906 7.188 22.906 7.188l4.063-26.625S640.99 439 628.03 439zM870 441.313l-25.031.062l-5.375 32.938s-9.41-12.782-24.125-12.782c-22.88 0-41.938 27.18-41.938 58.438c0 20.177 10.143 39.968 30.938 39.968c14.958 0 23.25-10.28 23.25-10.28l-1.094 8.78h24.313zm-716.938.343l-19.937 117.406h24.031l15.188-88.53l2.25 88.53h17.187l32.063-88.53l-14.219 88.53h25.531l19.688-117.406h-39.5l-24.594 72.031l-1.281-72.03h-36.406zm251.063 7.625c-.543.545-14.531 81.207-14.531 91.031c0 14.546 8.153 21.043 19.625 20.938c8.22-.07 14.586-2.135 17.531-3c.308-.083.906-.25.906-.25l3.094-21c-1.682 0-4.16.719-6.344.719c-8.565 0-9.514-4.557-8.969-7.938l6.907-42.843h13.031l3.156-23.22H426.25l2.5-14.437zm290.469 11.813c-16.773 0-29.625 5.531-29.625 5.531l-3.563 21.563s10.615-4.407 26.657-4.407c9.108 0 15.78 1.042 15.78 8.625c0 4.606-.812 6.313-.812 6.313s-7.202-.625-10.531-.625c-21.152 0-43.375 9.27-43.375 37.156c0 21.972 14.589 27 23.625 27c17.258 0 24.7-11.465 25.094-11.5l-.813 9.563h21.532l9.625-69c.003-29.278-24.934-30.22-33.594-30.22zm-403.907.125c-16.834 0-29.75 5.531-29.75 5.531l-3.562 21.563s10.65-4.407 26.75-4.407c9.142 0 15.844 1.046 15.844 8.625c0 4.604-.813 6.313-.813 6.313s-7.223-.625-10.562-.625c-21.233 0-43.531 9.255-43.531 37.125c0 21.96 14.618 27 23.687 27c17.321 0 24.79-11.466 25.188-11.5l-.813 9.562h21.625l9.656-68.968c0-29.261-25.027-30.22-33.719-30.22zm76.375 0c-18.077 0-36.437 7.297-36.437 32.219c0 28.238 30.594 25.385 30.594 37.28c0 7.94-8.538 8.595-15.125 8.595c-11.4 0-21.651-3.939-21.688-3.75l-3.281 21.562c.59.183 6.938 3.031 27.438 3.031c5.504 0 36.968 1.398 36.968-31.375c0-30.639-29.125-24.566-29.125-36.875c0-6.128 4.729-8.062 13.406-8.062c3.446 0 16.72 1.094 16.72 1.094l3.062-21.782c0 .003-8.565-1.937-22.531-1.937zm113 0c-25.12.005-43.718 27.017-43.718 57.531c0 35.219 23.302 43.594 43.156 43.594c18.326 0 26.406-4.094 26.406-4.094l4.375-24.031s-13.941 6.125-26.531 6.125c-26.826 0-22.125-19.969-22.125-19.969h50.781s3.282-16.138 3.282-22.719c0-16.42-8.195-36.437-35.625-36.437zm90.97 1.031c-11.317 0-19.72 15.219-19.72 15.219l2.25-13.969h-23.5l-15.78 96.438h25.968c7.355-41.182 8.698-74.621 26.25-68.5c3.072-15.94 6.053-22.098 9.406-28.844c0 0-1.575-.344-4.875-.344zm210.405 0c-11.316 0-19.718 15.219-19.718 15.219l2.25-13.969h-23.5l-15.782 96.438h25.938c7.358-41.182 8.735-74.621 26.281-68.5c3.078-15.94 6.056-22.098 9.407-28.844c0 0-1.576-.344-4.875-.344zm-301.5 20.719c14.267 0 11.657 16.104 11.657 17.406H463.5c0-1.662 2.658-17.406 16.438-17.406zm339.47 1.875c9.08-.003 13.75 6.131 13.75 20.562c0 13.094-6.604 30.594-20.282 30.594c-9.078 0-13.344-7.497-13.344-19.25c0-19.218 8.792-31.906 19.875-31.906zm-124.782 32.812c3.704.005 4.47.365 5.094.532c-.09-.015-.107.002.25.062c.469 4.295-2.595 24.438-17.407 24.438c-7.637 0-9.593-6.073-9.593-9.657c0-6.988 3.638-15.375 21.656-15.375m-403.906.094c4.207.005 4.666.475 5.375.594c.47 4.293-2.606 24.406-17.469 24.406c-7.667 0-9.656-6.044-9.656-9.625c0-6.984 3.66-15.375 21.75-15.375"/>',
    "odnoklassniki":'<path fill="currentColor" d="M289.656 0C147.32 0 31.531 115.771 31.531 258.094c0 142.312 115.789 258.031 258.125 258.031c142.331 0 258.063-115.719 258.063-258.031C547.719 115.771 431.988 0 289.656 0m0 151.219c58.909 0 106.813 47.967 106.813 106.875c0 58.883-47.903 106.813-106.813 106.813c-58.921 0-106.875-47.929-106.875-106.813c0-58.908 47.954-106.875 106.875-106.875M76.844 524.906c-25.538-.397-50.651 12.191-65.219 35.406c-22.263 35.367-11.598 82.036 23.688 104.281c46.674 29.282 97.321 50.057 149.844 62.031L40.907 870.937c-29.53 29.544-29.505 77.393.031 106.938C55.742 992.62 75.05 1000 94.407 1000c19.337 0 38.71-7.391 53.469-22.156l141.719-141.781l141.844 141.781c29.505 29.536 77.349 29.536 106.906 0c29.543-29.525 29.543-77.425 0-106.906L394.032 726.657c52.538-11.975 103.202-32.761 149.844-62.063c35.356-22.245 46.037-68.965 23.781-104.281c-22.281-35.375-68.931-46.031-104.313-23.75c-105.672 66.469-241.83 66.425-347.469 0c-12.16-7.659-25.654-11.448-39.031-11.656z"/>',
    "odnoklassniki-rect":'<path fill="currentColor" d="M140.719 0C62.769 0 0 62.769 0 140.719v718.563c0 77.95 62.769 140.719 140.719 140.719h718.563c77.95 0 140.719-62.769 140.719-140.719V140.719C1000.001 62.769 937.232 0 859.282 0zm381.688 102.188c113.484 0 205.75 92.304 205.75 205.781c0 113.468-92.266 205.719-205.75 205.719c-113.487 0-205.813-92.251-205.813-205.719c0-113.477 92.325-205.781 205.813-205.781m-2.188 120.563c-45.968 1.17-83.031 38.984-83.031 85.219c0 46.949 38.24 85.156 85.219 85.156c46.97 0 85.156-38.207 85.156-85.156c0-46.968-38.187-85.219-85.156-85.219c-.734 0-1.457-.019-2.188 0M352.031 520.689c10.9.037 21.897 3.036 31.813 9.281c84.228 52.962 192.808 52.997 277.063 0c28.21-17.765 65.391-9.237 83.156 18.969c17.745 28.159 9.221 65.389-18.969 83.125a385.4 385.4 0 0 1-119.469 49.469l115.063 115.063c23.555 23.506 23.555 61.709 0 85.25c-23.567 23.55-61.694 23.55-85.219 0L522.344 768.783l-113 113.063c-11.767 11.772-27.207 17.656-42.625 17.656c-15.434 0-30.822-5.9-42.625-17.656c-23.551-23.556-23.544-61.694 0-85.25l115-115.063c-41.878-9.548-82.286-26.122-119.5-49.469c-28.134-17.736-36.594-54.926-18.844-83.125c11.062-17.628 29.75-27.586 49.094-28.219a59 59 0 0 1 2.188-.031z"/>',
    "picasa":'<path fill="currentColor" d="M471.906 0c-55.299-.138-105.734 11.429-156.031 22.281l374.875 341.25l-5.156-299.875C603.522 15.357 534.578.156 471.906 0M238.343 55.875C-23.085 238.568-3.291 421.276 3.062 603.969l413.656-380.031zm517.031 54.313l2.594 555.844l142.188-5.188C965.65 400.1 881.464 240.929 755.375 110.188zM189.218 521.251L21.155 684.126c65.797 149.79 118.304 193.268 170.625 235.25zm87.875 204.219l-2.563 243.031c360.949 89.352 497.413-57.599 599.781-240.438l-597.219-2.594z"/>',
    "skype":'<path fill="currentColor" d="M958.234 572.394a463 463 0 0 0 5.7-72.422c0-256.241-207.709-463.95-463.962-463.95c-24.588 0-48.81 2.022-72.399 5.711C384.84 15.3 334.407 0 280.364 0C125.521 0 0 125.51 0 280.364c0 54.032 15.311 104.421 41.733 147.242c-3.655 23.645-5.667 47.722-5.667 72.377c0 256.253 207.709 463.906 463.895 463.906c24.611 0 48.833-1.9 72.422-5.622C615.16 984.656 665.548 1000 719.592 1000c154.876 0 280.386-125.543 280.386-280.397c.011-54.044-15.289-104.432-41.744-147.209M720.248 710.981c-19.633 28.255-48.455 50.333-86.133 66.21c-37.755 15.911-82.366 23.833-133.887 23.833c-61.755 0-112.888-10.799-153.243-32.621c-28.611-15.622-51.877-36.589-69.766-62.733c-17.922-26.155-27.011-51.766-27.011-76.744c0-14.733 5.623-27.555 16.556-37.999c11.055-10.522 25.288-15.8 42.088-15.8c13.767 0 25.589 4.144 35.211 12.355c9.555 8.211 17.566 20.145 24.122 35.9c7.855 18.255 16.389 33.511 25.466 45.677c9.022 12.022 21.722 22.067 38.166 29.922c16.267 7.878 37.911 11.811 65 11.811c37.066 0 67.032-8.011 89.954-23.833c23.044-15.955 34.122-35.377 34.122-58.855c0-18.711-5.978-33.588-18.211-45.177c-12.344-11.733-28.299-20.678-48.088-26.911c-19.867-6.3-46.544-12.989-79.977-20.033c-44.888-9.789-82.432-21.2-112.832-34.289c-30.444-13.155-54.688-31.121-72.655-53.821c-18.011-22.922-26.988-51.411-26.988-85.155c0-32.177 9.477-60.955 28.455-85.999c18.966-25.111 46.41-44.333 82.188-57.721c35.655-13.389 77.632-20.078 125.721-20.078c38.455 0 71.721 4.444 99.876 13.3c28.089 8.922 51.489 20.689 70.199 35.455c18.611 14.733 32.289 30.277 40.956 46.588c8.655 16.356 13.022 32.333 13.022 47.922c0 14.5-5.622 27.644-16.567 39.166c-10.955 11.545-24.955 17.422-41.31 17.367c-14.745 0-26.311-3.4-34.333-10.6c-7.811-6.967-16.011-18.055-24.889-33.511c-11.211-21.566-24.644-38.366-40.277-50.388c-15.211-11.789-40.311-17.933-75.233-17.856c-32.311 0-58.166 6.6-77.677 19.511c-19.633 13.122-29.022 28.345-29.066 46.178c.033 11.177 3.244 20.599 9.711 28.655c6.566 8.133 15.7 15.177 27.444 21.122c11.744 6.033 23.689 10.7 35.7 14.055c12.144 3.445 32.299 8.422 60.31 15.011c35.133 7.578 67.01 16.033 95.643 25.255c28.578 9.267 52.967 20.445 73.122 33.756c20.311 13.277 36.188 30.199 47.522 50.644c11.399 20.477 17.088 45.488 17.088 74.91c.045 35.444-9.833 67.31-29.499 95.521"/>',
    "tudou":'<path fill="currentColor" d="M499.084-.344c-13.894.007-28.131.6-42.719 1.781C229.306 19.83 35.221 194.053 4.646 426.156c-39.01 296.18 172.832 550.084 440.156 571.406c211.837 16.895 347.632-70.649 440.125-181.469c80.796-96.833 130.43-238.563 108.094-393.813c-20.793-144.786-124.917-284.666-247.125-355.219C676.935 27.241 596.341-.391 499.083-.345zm2.344 96.125c50.071.253 102.426 9.433 155.688 29.219c136.812 73.88 246.102 182.688 247.094 370.625c1.167 215.208-151.264 391.086-366.781 409.281C302 924.796 47.61 730.894 97.304 430C127.809 245.25 295.138 94.739 501.429 95.781zm183.219 246.094c-66.028.501-132.245 41.791-127.906 122.875c60.795-39.627 171.351-47.17 254.813-15.406c-3.02-72.403-64.88-107.939-126.906-107.469zm-374.281.813c-60.743.366-120.45 34.43-124.25 106.656c36.786-4.428 77.267-25.621 111.938-19.344c-14.141 32.182-29.441 63.213-34.75 104.25c-42.446-43.943-95.643 35.348-42.469 61.781c-38.835 8.128-24.873 74.565 15.438 69.5c13.635-5.684 23.061-15.556 34.781-23.156c28.371 64.563 104.058-24.754 42.469-50.219c60.972-27.315-11.298-97.126-38.656-61.75c18.194-35.684 19.3-91.529 57.938-100.406c36.256-8.326 70.788 25.741 108.125 34.75c8.449-79.621-61.72-122.477-130.563-122.063zm-43.781 238.375c4.172-.238 9.65 3.631 16 14.969c-29.327 30.254-29.629-14.191-16-14.969"/>',
    "tumblr":'<path fill="currentColor" d="M345.603 0v252.756h237.3v156.983h-237.3v256.442c0 57.96 3.061 95.177 9.218 111.653c6.122 16.446 17.571 29.584 34.251 39.414c22.126 13.261 47.413 19.907 75.914 19.907c50.608 0 100.978-16.476 151.073-49.398V945.47c-42.746 20.124-81.441 34.283-116.056 42.382c-34.653 8.099-72.119 12.149-112.384 12.149c-45.694 0-86.108-5.782-121.238-17.312c-35.128-11.561-65.082-28.037-89.897-49.366c-24.817-21.392-42.016-44.111-51.587-68.192c-9.583-24.051-14.374-58.949-14.374-104.639V409.739H0V268.224c39.276-12.766 72.958-31.048 100.94-54.899c28.025-23.815 50.493-52.44 67.432-85.855C185.335 94.085 196.997 51.58 203.37.001h142.232"/>',
    "tumblr-rect":'<path fill="currentColor" d="M192.906 0C86.033 0 0 86.033 0 192.906v614.188C0 913.967 86.033 1000 192.906 1000h614.188C913.967 1000 1000 913.967 1000 807.094V192.906C1000 86.033 913.967 0 807.094 0zm242.031 141.406h99.125v176.188H699.5v109.438H534.062v178.75c0 40.402 2.146 66.359 6.438 77.844c4.267 11.464 12.248 20.616 23.875 27.469c15.423 9.244 33.039 13.875 52.906 13.875c35.278 0 70.392-11.489 105.313-34.438V800.47c-29.797 14.028-56.746 23.917-80.875 29.563c-24.156 5.645-50.276 8.469-78.344 8.469c-31.852 0-60.043-4.057-84.531-12.094c-24.487-8.059-45.358-19.539-62.656-34.406c-17.299-14.912-29.297-30.745-35.969-47.531c-6.68-16.765-10-41.089-10-72.938v-244.5h-77.063v-98.656c27.378-8.899 50.87-21.624 70.375-38.25c19.535-16.601 35.192-36.551 47-59.844c11.825-23.272 19.963-52.921 24.406-88.875z"/>',
    "twitter":'<path fill="currentColor" d="M748.681 875.015c0 34.315-12.307 63.755-36.922 88.227c-24.615 24.519-54.117 36.758-88.576 36.758H375.078c-103.368 0-191.702-36.519-265.055-109.647C36.65 817.234.001 729.224.001 626.187v-501.08c0-35.311 12.219-64.969 36.649-89.031C61.08 12.049 90.923.001 126.146.001c34.199 0 63.584 12.273 87.981 36.791c24.471 24.492 36.724 53.878 36.724 88.228v181.436h353.233c32.069 0 59.545 11.366 82.466 34.116c22.893 22.71 34.315 49.967 34.315 81.797c0 31.769-11.421 59.073-34.233 81.802c-22.82 22.698-50.239 34.083-82.173 34.083H250.851v87.817c0 34.438 12.021 63.687 36.184 87.817c24.136 24.063 53.441 36.109 87.918 36.109h248.182c34.438 0 64.008 12.287 88.623 36.793q36.922 36.776 36.922 88.226"/>',
    "twitter-bird":'<path fill="currentColor" d="M1231.051 118.453q-51.422 76.487-126.173 130.403q.738 14.46.738 32.687q0 101.273-29.53 202.791q-29.53 101.519-90.215 194.343T841.297 843.145T639.62 957.395t-252.474 42.606q-210.2 0-387.147-113.493q31.406 3.495 60.242 3.495q175.605 0 313.687-108.177q-81.877-1.501-146.654-50.409q-64.777-48.907-89.156-124.988q24.097 4.59 47.566 4.59q33.782 0 66.482-8.812q-87.378-17.5-144.975-87.04q-57.595-69.539-57.595-160.523v-3.126q53.633 29.696 114.416 31.592q-51.762-34.508-82.079-89.999q-30.319-55.491-30.319-120.102q0-68.143 34.151-126.908q95.022 116.607 230.278 186.392q135.258 69.786 290.212 77.514q-6.609-27.543-6.621-57.485q0-104.546 73.994-178.534Q747.623 0 852.169 0q109.456 0 184.392 79.711q85.618-16.959 160.333-61.349q-28.785 90.59-110.933 139.768q75.502-8.972 145.088-39.677z"/>',
    "vimeo":'<path fill="currentColor" d="M1158.195 231.332q-7.791 168.795-236.313 462.239q-236.313 306.428-399.915 306.428q-101.277 0-171.392-186.973l-93.487-342.784q-51.937-186.973-111.665-186.973q-12.984 0-90.89 54.534L-.001 267.688q85.696-75.309 168.795-150.617q114.261-98.68 171.392-103.874Q475.222.213 506.384 197.573q33.759 212.942 46.743 264.879q38.953 176.586 85.696 176.586q36.356 0 109.068-114.261t77.906-173.989q10.387-98.68-77.906-98.68q-41.55 0-85.696 18.178Q747.891-7.577 989.398.214q179.183 5.194 168.795 231.12z"/>',
    "vimeo-rect":'<path fill="currentColor" d="M192.906 0C86.033 0 0 86.033 0 192.906v614.188C0 913.967 86.033 1000 192.906 1000h614.188C913.967 1000 1000 913.967 1000 807.094V192.906C1000 86.033 913.967 0 807.094 0zm516.719 196.969c3.044-.015 6.112.025 9.219.125c73.759 2.138 108.495 49.688 104.219 142.688c-3.207 69.483-51.837 164.644-145.906 285.438c-97.276 126.138-179.593 189.188-246.938 189.188c-41.69 0-76.982-38.472-105.844-115.438l-57.719-211.656c-21.379-76.966-44.351-115.438-68.938-115.438c-5.345 0-24.056 11.208-56.125 33.656l-33.688-43.281c35.276-31 70.043-62 104.25-93c47.034-40.621 82.295-62.018 105.813-64.156q83.381-8.018 102.625 113.844c13.897 87.655 23.53 142.183 28.875 163.563c16.034 72.69 33.665 109.031 52.906 109.031c14.966 0 37.413-23.528 67.344-70.563c29.931-47.034 45.956-82.82 48.094-107.406c4.276-40.621-11.749-60.938-48.094-60.938c-17.103 0-34.734 3.736-52.906 11.219c34.173-110.805 98.447-166.422 192.813-166.875z"/>',
    "visa":'<path fill="currentColor" d="M40 186c-22.087 0-40 17.908-40 40v548c0 22.091 17.922 40 40 40h920c22.087 0 40-17.908 40-40V226c0-22.091-17.922-40-40-40zm557.25 187.156c24.338 0 43.843 5.376 56.281 10.375l-8.5 53.469l-5.625-2.844c-11.587-4.999-26.47-9.81-47-9.469c-24.576 0-35.937 10.963-35.937 21.22c-.143 11.557 13.275 19.179 35.218 30.593c36.217 17.61 52.956 38.962 52.72 67.031c-.488 51.217-43.324 84.313-109.313 84.313c-28.153-.31-55.276-6.27-69.938-13.157l8.813-55.25l8.094 3.938c20.617 9.212 33.967 12.938 59.093 12.938c18.043 0 37.409-7.555 37.563-24.094c.117-10.8-8.095-18.5-32.532-30.594c-23.812-11.804-55.38-31.577-55.03-67.031c.372-47.961 44.063-81.438 106.093-81.438M88.437 381h101.344c13.647.516 24.665 4.91 28.469 19.719l21.844 112.75c.003.01-.004.051 0 .062l6.562 33.782L308.187 381h66.563l-98.938 243.438l-66.5.062l-52.937-196.438c31.498 16.687 58.314 35.994 73.844 62.563c-4.004-8.407-9.28-17.897-16.031-27.25c-7.862-10.892-24.841-24.955-31.938-30.938c-24.73-20.847-58.315-37.684-94.594-46.593zm312.782.281h65.094l-40.72 243.063H360.5zm367.656 0h49.313l51.625 243.063h-59.188s-5.872-27.924-7.781-36.438c-9.303 0-74.367-.093-81.688-.093c-2.477 6.58-13.437 36.53-13.437 36.53H640.75l94.719-222.874c6.706-15.838 18.137-20.188 33.406-20.188m4.844 65.375c-3.203 9.041-8.78 23.641-8.407 23c0 0-20.037 53.44-25.28 67.313l52.655-.032A191956 191956 0 0 0 778 467.408l-4.281-20.75z"/>',
    "vkontakte-rect":'<path fill="currentColor" d="M196.031.063C88.252.063.062 88.253.062 196.032V804.22c0 107.779 88.19 195.969 195.969 195.969h608.188c107.779 0 195.969-88.19 195.969-195.969V196.032c0-107.779-88.19-195.969-195.969-195.969zm112.188 225.656H505.5c40.972 0 72.094 1.499 93.219 4.719c21.125 3.059 40.968 9.616 59.406 19.594c19.953 10.749 34.941 25.224 44.813 43.156c9.87 18.038 14.875 38.771 14.875 62.344c0 27.244-6.798 51.461-20.5 72.719c-13.57 21.258-32.399 36.89-56.344 47v2.906c34.454 7.317 62.087 22.036 82.813 44.438c20.757 22.428 31.125 52.618 31.125 90.531c0 27.75-5.239 52.227-15.75 73.219c-10.507 20.992-24.577 38.29-42.375 52.125c-20.992 16.496-44.033 28.137-69.281 35.188c-25.089 7.024-56.959 10.531-95.75 10.531H308.22V225.72zM447.75 328.094v118.188h17.656c23.945 0 40.864-.246 50.469-.75s19.601-3.077 29.844-7.813c11.121-5.241 18.953-12.518 23.156-21.75c4.23-9.339 6.406-20.123 6.406-32.281c0-9.072-2.308-18.31-6.938-27.781c-4.603-9.472-11.754-16.395-21.625-20.625c-9.206-4.097-20.091-6.257-32.781-6.656c-12.664-.4-31.482-.531-56.438-.531zm0 213.844v139.813h7.563c36.503 0 61.715-.275 75.656-.781c13.941-.504 28.303-3.815 42.75-10.094c12.824-5.507 22.031-13.577 27.938-24.219c5.88-10.616 8.813-22.658 8.813-36.094c0-17.294-3.418-30.731-10.469-40.469c-7.024-9.844-17.532-17.154-31.5-22.156c-8.461-3.326-20.248-5.128-35.094-5.5s-34.799-.5-59.781-.5z"/>',
    "win8":'<path fill="currentColor" d="m985.875 0l-548.75 80.031v397.688h548.75zM398.469 85.688L0 143.813v333.906h398.469zM0 516.376v338.156l398.469 58.813V516.376zm437.125 0v402.688l548.75 80.938V516.377z"/>',
    "wordpress":'<path fill="currentColor" d="M501.983 0C306.099-.341 121.728 115.812 37.114 299.669l237.919.566v26.02c-73.261-8.816-66.81 29.228-56.439 69.607l156.183 391.429l114.795-269.092l-52.699-126.108c-19.142-33.777-19.546-71.25-97.858-67.721l1.1-23.978l348.943.849v26.869c-40.865 2.397-90.057-10.274-63.981 73.409l141.13 370.72l77.149-216.424c25.263-90.724 1.487-177.91-28.534-263.405c-16.697-57.355-8.91-105.254 65.93-124.255C766.517 51.55 631.727.226 501.987 0zm474.58 342.25L750.334 931.939c62.476-54.11 126.535-73.806 204.922-223.526c52.155-124.532 46.786-239.897 21.307-366.164zm-963.395 9.553C-8.913 525.888-6.893 576.876 47.767 719.036c71.25 136.154 126.502 164.84 204.922 218.246zm497.646 220.886L343.162 977.191c117.681 33.589 222.978 29.385 319.372-7.982z"/>',
    "yandex":'<path fill="currentColor" d="M456.102 544.012H388.14c-106.741 0-210.255-63.998-210.255-224.006c0-166.693 97.036-234.679 195.696-234.679h82.52zM562.843 0H378.434C197.307 0 43.64 113.334 43.64 333.342c0 132.002 74.429 229.329 207.016 277.33L3.235 978.679c-8.081 12.009 0 21.321 12.901 21.321h114.833c9.706 0 16.183-2.662 19.411-9.312l224.814-361.357H456.1v361.357c0 3.988 4.853 9.312 11.276 9.312h100.318c9.706 0 12.945-3.997 12.945-10.638V13.344C580.639 3.988 574.161 0 562.842 0"/>',
    "yandex-rect":'<path fill="currentColor" d="M177.188 0C79.032 0 0 79.032 0 177.188v645.625c0 98.156 79.032 177.188 177.188 177.188h475.188l347.625-499.875L652.376.001zm266.094 182.563h112.906c6.932 0 10.906 2.535 10.906 8.5v622.25c0 4.234-1.995 6.781-7.938 6.781H497.75c-3.934 0-6.906-3.396-6.906-5.938V583.781h-49.563L303.625 814.156c-1.976 4.239-5.932 5.938-11.875 5.938h-70.313c-7.9 0-12.854-5.937-7.906-13.594l151.5-234.625c-81.187-30.602-126.75-92.656-126.75-176.813c0-140.264 94.091-212.5 205-212.5zm-2.969 54.375c-60.412 0-119.844 43.352-119.844 149.625c0 102.011 63.389 142.813 128.75 142.813h41.625V236.938z"/>',
    "youku":'<path fill="currentColor" d="M449.69 139.688c-15.29.025-30.615 4.098-43.781 13.188c-28.992 20.037-45.122 81.793-64.594 125.75c-21.444 48.358-40.259 97.095-61.156 132.594c-56.519-64.128-95.077-206.254-163.156-258.344c-21.959-16.783-70.099-16.305-91.781 0c-58.645 44.149.194 122.203 23.781 163.156c51.773 89.789 109.164 185.937 152.969 268.531c-28.476 75.092-172.29 287.292 20.406 275.313c46.636-30.14 66.384-92.854 91.75-149.594c51.861-116.102 95.923-216.406 149.563-336.469c22.294-49.908 76-123.645 64.594-180.156c-6.391-31.499-42.38-54.028-78.594-53.969zm466.219 46.063c-27.866.279-55.968 13.293-68.125 38.5c-20.112 22.945-41.65 44.45-64.594 64.563c-11.023 13.893-23.501 26.333-37.375 37.375c-16.133 16.783-29.222 36.525-51 47.625V241.251c-.497-72.107-135.42-72.107-135.938 0v377.313c5.893 26.925 18.13 47.511 47.563 50.938h40.813c27.977-4.861 43.889-21.793 47.563-50.938V536.97c0-28.284-5.294-76.032 13.594-47.594c27.691 27.098 66.712 56.252 102 88.344c33.872 30.887 66.285 80.739 101.938 88.375c73.005 15.653 105.208-54.421 78.188-105.344c-28.534-53.832-110.717-88.282-146.156-135.969c23.883-54.654 153.715-88.23 142.75-183.531c-4.322-37.644-37.587-55.837-71.219-55.5z"/>',

};
}